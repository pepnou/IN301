...
void lancement_option_2(int argc, char **argv, SDL_Surface * ecran)
{
	...
	if (!(strcmp(&(argv[3][strlen(argv[3]) - strlen(extension_fichier)]), extension_fichier))) {
		...
		if (NBR_NIVEAUX >= lvl_a_lire)
			play_mode(ecran, lvl_a_lire);
		else {
			printf("le niveau que vous avez rentré n'existe pas\n");
		}
	} else
		printf("le fichier passé en option ne posséde pas la bonne extension (.xsb)\n");
}
...
void lancement_base(int argc, SDL_Surface * ecran)
{
	if (argc > 1) {
		...
		printf
		    ("Pour information , l'extension des fichier doit necessairement etre \'xsb\' , et le format des option doit etre fichier.xsb ou -n num fichier.xsb ou -c fichier.xsb\n");
...
{
	if ((argc >= 2)
	    && (!(strcmp(&(argv[1][strlen(argv[1]) - strlen(extension_fichier)]), extension_fichier)))) {

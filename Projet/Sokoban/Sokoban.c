...
int changement_case(int valeur_case)
{
	switch (valeur_case) {
	case VIDE:
		return MUR;
...
void play_mode(SDL_Surface * ecran, int num_lvl)
{
	...
	if (evenement.event == E_QUIT)
		main_menu(ecran);
	else if (coup.num_level < NBR_NIVEAUX)
		play_mode(ecran, coup.num_level + 1);
	else
		main_menu(ecran);
}
